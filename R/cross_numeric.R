...
summarize_numeric_factor = function(x, by, funs, funs_arg, showNA, total, 
    ...
    # x=x[by_filter]
    # by=by[by_filter]
    # grp = expand.grid(x = unique(x), by = unique(by))
    # 
    # 
    # 
    # map2(x, by, ~{
    ...
    # xx=bench::mark(
    # xx=microbenchmark::microbenchmark(
    #     times=100,
    #     by=by(x[by_filter], by[by_filter], summarize_numeric_single, funs=funs, funs_arg=funs_arg) %>% 
    #         imap_dfr(~{
